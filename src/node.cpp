...
MonteCarloTree::Node::Node(Node *parent, int index, GameStatus parentStatus)
    : index_(index),
      parent_(parent),
      child_(NULL),
      next_(NULL),
...
MonteCarloTree::Node::Node(Node *parent, const Node *source)
...
void MonteCarloTree::Node::deleteChildren(MemoryPool* pool) {
...
void MonteCarloTree::Node::deleteChildrenExcept(Node* exceptNode, MemoryPool* pool) {
  Node *child = child_, *next;
...
std::pair<SearchStatus, MonteCarloTree::Node*> MonteCarloTree::Node::selection(
...
MonteCarloTree::Node* MonteCarloTree::Node::child(int index) const {
  for (Node* node : *this)
...
MonteCarloTree::Node* MonteCarloTree::Node::newChild(
...
MonteCarloTree::Node* MonteCarloTree::Node::newChild(Node* source, MemoryPool* pool) {
...
void MonteCarloTree::Node::minus(const Node* node) {
  count_ -= node->count_;
  winLoseCount_ -= node->winLoseCount_;
}

void MonteCarloTree::Node::merge(const Node* node) {
  count_ += node->count_;
  winLoseCount_ += node->winLoseCount_;
...
double MonteCarloTree::Node::getUCBValue(const Node* node) const {
